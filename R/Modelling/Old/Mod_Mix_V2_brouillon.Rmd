...
print(paste(c("cst_variance :",cst_var,'||',"percentage BMGKT :",prob_type[1],'||',"percentage GM ",prob_type[2],"percentageT ",prob_type[3],'||',"cst_ab for sampling :",cst_ab),collapse = " "))
...
pop_df$cellCount=rowSums(pop_df[,cells])
 ...
 F=round(prop.table(table(prop$type))*100)
 prop$type=factor(prop$type,levels=c('GMBKT','GM','T'))
 prop$type_percent=factor(prop$type,levels=c('GMBKT','GM','T'),labels = c(paste0('GMBKT - ',F['GMBKT'],'%'),
                                                                          paste0('GM - ',F['GM'],'%'),
                                                                          paste0('T - ',F['T'],'%')))
 
  
 samp= ggtern(prop,aes(G+M,T,B+K))+
