...
text_low <- grobTree(textGrob(paste0(n_c1,'\nbias'), x=0.04,  y=-0.18, gp=gpar(fontsize=40, fontface="bold")))
text_high <- grobTree(textGrob(paste0(n_c2,'\nbias'), x=0.96,  y=-0.18, gp=gpar(fontsize=40, fontface="bold")))
bal <- grobTree(textGrob('Bal.', x=0.5,  y=-0.18, gp=gpar(fontsize=40, fontface="bold")))
Larrow<-grobTree(linesGrob(x = unit(c(0.10, 0.45), "npc"),
               y = unit(c(-0.18, -0.18), "npc"),
               default.units = "npc",arrow=arrow(type="open", ends="first", length=unit(3,"mm")),gp=gpar(col="black", lwd=4)))
Rarrow<-grobTree(linesGrob(x = unit(c(0.55, 0.90), "npc"),
               y = unit(c(-0.18, -0.18), "npc"),
               default.units = "npc",arrow=arrow(type="open", ends="last", length=unit(3,"mm")),gp=gpar(col="black", lwd=4)))
 ...
 # facet_wrap(~pat,nrow=1,labeller = label_parsed,scales='free')+
  ...
  coord_cartesian(clip = "off")
   ...
   grid.arrange(G,top=textGrob(paste(gsub('_.*','',c1),'vs',gsub('_.*','',c2),'lineage bias'),gp=gpar(fontsize=38,font=2)))
...
print(paste(c("cst_variance :",cst_var,'||',"percentage GMBKT :",prob_type[1],'||',"percentage GM ",prob_type[2],"percentageT ",prob_type[3],'||',"cst_ab for sampling :",cst_ab),collapse = " "))
...
pop_df$cellCount=rowSums(pop_df[,cells])
 ...
 colnames(prop)=c('B','M','G','K','T','cellCount','type','posid')
 prop$type_percent=factor(prop$type,levels='GMBKT',labels = paste0('GMBKT - ',prop.table(table(prop$type))*100,'%'))
 ...
 samp= ggtern(prop,aes(G+M,T,B+K,group=type))+
      ...
      scale_color_manual(labels=prop$type_percent,values = colors)+
...
```{r Pipeline, echo=FALSE, message=FALSE,  fig.height=9, fig.width=20,dev='png',dpi=350}
